...
try:
    ...
    from relevance_detection import RelevanceMonitor, RelevanceMatch
...
def display_news_dashboard(G, openai_api_key: str):
    """Display the news monitoring dashboard"""
...
def display_pipeline_control(G, openai_api_key: str):
    ...
    with col1:
        ...
        max_articles = st.slider("Max articles to fetch:", 10, 500, 100)
    ...
    with col2:
        ...
        if auto_refresh:
            refresh_interval = st.selectbox("Refresh every:", ["15 min", "30 min", "1 hour", "4 hours"])
    ...
    with col3:
        if st.button("🔄 Full Update"):
            if st.session_state.news_pipeline and G is not None:
                with st.spinner("Running full update..."):
                    # Run both pipeline and relevance analysis
                    run_news_pipeline(max_articles, openai_api_key)
                    analyze_relevance(G, days_back, min_relevance, openai_api_key)
                    st.success("✅ Full update complete!")
            else:
                st.error("Please initialize the news system and ensure knowledge graph is loaded")
